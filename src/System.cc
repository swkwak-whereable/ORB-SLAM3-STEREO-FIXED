...
void System::GetOdomInfos(std::vector<Sophus::SE3f>& poses,
	...
	// Tracker 로그가 비어있으면 바로 종료
	if (!mpTracker) return;
	if (mpTracker->mlRelativeFramePoses.empty()) return;

	// KeyFrame이 없으면 기준좌표(Tow)를 잡을 수 없으므로 종료
	std::vector<KeyFrame*> vpKFs = mpAtlas->GetAllKeyFrames();
	if (vpKFs.empty()) return;

	std::sort(vpKFs.begin(), vpKFs.end(), KeyFrame::lId);

	// 첫 KeyFrame이 원점이 되도록 정렬하는 변환 (SaveTrajectoryTUM과 동일한 의도)
	// (loop closure 이후 첫 KF가 원점이 아닐 수 있어서)
	Sophus::SE3f Tow = vpKFs[0]->GetPoseInverse(); // world->? 기준 맞추기용 :contentReference[oaicite:2]{index=2}

	// 리스트들(프레임 단위 로그)은 서로 같은 길이로 쌓인다는 가정
	...
	for (auto lit = mpTracker->mlRelativeFramePoses.begin(),
		...
		lit != lend;
		++lit, ++lRit, ++lT, ++lbL) {
		const double t = (lT != mpTracker->mlFrameTimes.end()) ? *lT : 0.0;
		const bool lost = (lbL != mpTracker->mlbLost.end()) ? *lbL : true;

		vTimestamps.push_back(t);
		vValid.push_back(!lost);

		// tracking 실패 프레임은 identity로 채우고(valid=false) 넘어감
		if (lost || lRit == mpTracker->mlpReferences.end() || (*lRit) == nullptr) {
			poses.emplace_back(); // Sophus::SE3f() : identity (기본 생성자) :contentReference[oaicite:3]{index=3}
			continue;
		}

		KeyFrame* pKF = *lRit;

		// KF가 cull되었으면 spanning tree를 타고 올라가며 보정(Trw 누적)
		Sophus::SE3f Trw; // identity :contentReference[oaicite:4]{index=4}
		while (pKF && pKF->isBad()) {
			Trw = Trw * pKF->mTcp;
			pKF = pKF->GetParent();
		}
		...
		// SaveTrajectoryTUM과 동일한 형태로 "reference KF의 월드포즈"를 붙여서
		// 프레임의 절대 포즈를 복원
		Trw = Trw * pKF->GetPose() * Tow; // KF -> world (정확한 의미는 ORB 내부 convention 따름) :contentReference[oaicite:5]{index=5}
		Sophus::SE3f Tcw = (*lit) * Trw;  // frame: camera<-world
		Sophus::SE3f Twc = Tcw.inverse(); // frame: world<-camera

		poses.push_back(Twc);
	}

	// 주의:
	// - MONOCULAR일 때 translation은 "임의 스케일"이며 drift가 있음.
	// - 하지만 rotation, 그리고 (상대) motion direction은 여전히 유용할 수 있음.
	// ORB-SLAM3가 원래 SaveTrajectoryTUM을 mono에서 막는 이유가 scale 문제임. :contentReference[oaicite:6]{index=6}
}
...
// 			poses.push_back(Sophus::SE3f::Identity());
// 			continue;
// 		}
